  ...
  useGSAP(
    () => {
      ...
      ScrollTrigger.getAll().forEach((st) => {
        if (
          st.vars.trigger &&
          containerRef.current?.contains(st.vars.trigger as Element)
        ) {
          st.kill();
        }
      });
      ...
      const allAnimatedElements = containerRef.current.querySelectorAll(
...
2. **Transición suave opcional** con flag en `localStorage`:
   ...
   ```typescript
   const preferSmoothTransition =
     localStorage.getItem("smoothLocaleSwitch") === "true";
