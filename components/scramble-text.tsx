...
export function ScrambleText({ text, className, delayMs = 0, duration = 0.9 }: ScrambleTextProps) {
  ...
  }, [text, delayMs, duration, hasAnimated]) // Empty deps - only run on mount
...
export function ScrambleTextOnHover({
  ...
  return (
    <Component className={className} onMouseEnter={handleMouseEnter} onClick={onClick} href={href} {...props}>
      {displayText}
    </Component>
  )
}
